try<- read.table("D:/zhuangyimin/Rroute/cazymeqipao.txt",header = T)  
library("ggplot2")
...
dev.new()
ggplot(try, aes(x = group, y = genus, ,color=genus,size = value)) + geom_point()+scale_color_viridis(discrete=TRUE, option="E")+
